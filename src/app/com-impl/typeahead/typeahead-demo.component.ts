...
import { autocompleteModel, hits, hit, fields } from "../../model/autocompleteModel";
...
export class TypeaheadDemoComponent {
  ...
  public states: Array<string> = ['Alabama', 'Alaska', 'Arizona', 'Arkansas', 'California', 'Colorado', 'Connecticut', 'Delaware', 'Florida', 'Georgia', 'Hawaii', 'Idaho',
    'Illinois', 'Indiana', 'Iowa', 'Kansas', 'Kentucky', 'Louisiana', 'Maine', 'Maryland', 'Massachusetts', 'Michigan', 'Minnesota',
    'Mississippi', 'Missouri', 'Montana', 'Nebraska', 'Nevada', 'New Hampshire', 'New Jersey', 'New Mexico', 'New York', 'North Dakota', 'North Carolina', 'Ohio', 'Oklahoma', 'Oregon',
  ...
  public getStatesAsObservable(token: string): Observable<any> {
    ...
    return Observable.of(
      ...
      // })
      this.result.filter((cities: any) => {
        debugger;
        return query.test(cities.fields.city_en);
      })
    );
  }

  getHotelListingOnLoad(): void {
    let cities: Array<string> =[];
    this.httpService.get('https://yl7ciftkyj.execute-api.eu-west-1.amazonaws.com/Dev/city/autocomplete?return=cityid,city_en,city_ar&q=du*&sort=&fq=&size=1500&q.options=')
      .map(res => <autocompleteModel>res.json()).subscribe(data => {
        //console.log(data);
        this.result = data.hits.hit;
        for (let entry of this.result) {
          cities.push(entry.fields.city_en);
        }
        console.log(cities);
      });
    //  this.httpService.get('http://echo.jsontest.com/key/value/one/two' ,options).map(res=>res.json()).catch(this.handleError);;
